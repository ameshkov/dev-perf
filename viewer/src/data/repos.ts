/**
 * Per-repository aggregation for the viewer: sums one repository's
 * commits, distinct users, LLM contributions, and language lines
 * across all periods of the report. Mirrors `src/compile/repos.ts` of
 * the parent CLI.
 */
 ...
 * distinctly. When the same repository URL is analyzed on several
 * branches (aliased specs), all branch entries contribute to the one
 * summary, so the totals and the per-period timeline cover the whole
 * repository.
 *
