/**
 * Transient-failure classification and backoff for git commands: which
 * git failures are worth retrying (a refused or timed-out connection, a
 * dropped remote) and how long to wait between attempts. `runGit` in
 ...
 * temporarily unreachable, a timed-out or dropped connection — so a
 * retry makes sense. Permanent failures (unknown repository, bad
 * credentials, an empty repository) are deliberately absent: retrying
 * them only wastes the backoff budget.
 */
 ...
 * host). Permanent failures — unknown repository, auth failure, an
 * empty repository — return false and fail immediately. `runGit`
 * consults this for every failed attempt.
 *
 ...
 * refused` — so the log names the actual fault, not git's generic
 * wrapper text. Falls back to the first stderr line when nothing
 * matches.
 *
 * @param error - The failed git invocation's error.
