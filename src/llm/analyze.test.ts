  ...
  contributions: [
    {
      ...
      commits: ['def5678a'],
      qualitySignals: ['regression test added'],
      riskFlags: [],
    },
  ],
};
 ...
 * — when `callTool` is set — every non-`noReply` prompt makes the
 * model "call" `devperf_report` by writing the session's report file.
 */
class StubSessions implements SessionService {
  ...
  constructor(
    private readonly state: {
      /** Where the simulated tool writes report files. */
      reportDir: string;
  ...
  async promptSession(
    ...
    if (!options?.noReply && this.state.callTool) {
      await writeJsonFile(path.join(this.state.reportDir, `${handle.id}.json`), this.state.payload);
    }
...
function stub(reportDir: string, callTool: boolean, payload = PAYLOAD_A): StubSessions {
...
describe('analyzeRepositoryLLM', () => {
  it('runs orientation once and one session per user, returning completed analyses', async () => {
    const service = stub(llmDir, true);
    const groups = [
  ...
  it('reruns with the same parameters without making a second call', async () => {
    ...
    const second = stub(llmDir, false, PAYLOAD_B);
  ...
  it('--refresh invalidates the cache and re-runs the analysis', async () => {
    const first = stub(llmDir, true, PAYLOAD_A);
    const groups = [group('alice@example.com', 'Alice', 'abc1234d', 'Add pipeline')];
    await analyzeRepositoryLLM(inputFor(first, groups));

    const refreshed = stub(llmDir, true, PAYLOAD_B);
  ...
  it('reuses cached results for users with hits while analyzing the rest', async () => {
    ...
    const second = stub(llmDir, true, PAYLOAD_B);
  ...
  it('enforces the report tool: 3 reminders, then an error naming user and session', async () => {
    const service = stub(llmDir, false);
    const groups = [group('alice@example.com', 'Alice', 'abc1234d', 'Add pipeline')];
